...
void SP_target_speaker (edict_t *ent)
{
	...
	if (!strstr (st.noise, ".wav"))
		...
		Q_strncpy (buffer, st.noise, sizeof(buffer));
...
{
	if (ent->spawnflags & 1)
		Q_strncpy (game.helpmessage1, ent->message, sizeof(game.helpmessage2)-1);
	else
		Q_strncpy (game.helpmessage2, ent->message, sizeof(game.helpmessage1)-1);
...
void use_target_blaster (edict_t *self, edict_t *other, edict_t *activator)
{
	...
	fire_blaster(self, self->s.origin, self->movedir, self->dmg, self->speed, effect, BLASTER_PROJ_BOLT, MOD_TARGET_BLASTER, 2.0, false);
